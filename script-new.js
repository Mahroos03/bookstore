// NETH Bookhive E-commerce JavaScript - Complete SRS Implementation
// Author: reezmahanan  
...
class BookHaven {
    ...
    init() {
    console.log('NETH Bookhive initializing...');
    ...
    this.setupModals();
    // Header toggles removed per user request (no toggle buttons in header)
    // Sidebars and notifications removed for minimal header
    this.setupFilters();
        this.setupViewOptions();
        this.updateBreadcrumb();
    // Start holiday countdown (counts down to Dec 31 of current year)
    this.startHolidayCountdown();
    // Page toggles (cart, wishlist, compare, notifications, delivery)
    this.setupPageToggles();
        
    console.log('NETH Bookhive initialization complete, hiding loading in 2.5 seconds...');
    ...
    // (toggle panel functions removed)
    ...
    setupPageToggles() {
        ...
        const panels = {
            ...
            wishlist: document.getElementById('wishlistPanel'),
            compare: document.getElementById('comparePanel'),
            notifications: document.getElementById('notificationsPanel'),
            delivery: document.getElementById('deliveryPanel')
        };

        const hideAll = () => Object.values(panels).forEach(p => p && (p.style.display = 'none'));
        ...
        if (cartToggle && panels.cart) cartToggle.addEventListener('click', (e) => {
            e.stopPropagation();
            const visible = panels.cart.style.display === 'block'; hideAll(); if (!visible) { this.renderCartPanel(); positionPanel(panels.cart, cartToggle); }
        });

        if (wishlistToggle && panels.wishlist) wishlistToggle.addEventListener('click', (e) => {
            e.stopPropagation(); const visible = panels.wishlist.style.display === 'block'; hideAll(); if (!visible) { this.renderWishlistPanel(); positionPanel(panels.wishlist, wishlistToggle); }
        });

        if (compareToggle && panels.compare) compareToggle.addEventListener('click', (e) => {
            e.stopPropagation(); const visible = panels.compare.style.display === 'block'; hideAll(); if (!visible) { this.renderComparePanel(); positionPanel(panels.compare, compareToggle); }
        });

        if (notifToggle && panels.notifications) notifToggle.addEventListener('click', (e) => {
            e.stopPropagation(); const visible = panels.notifications.style.display === 'block'; hideAll(); if (!visible) { positionPanel(panels.notifications, notifToggle); }
        });

        if (deliveryToggle && panels.delivery) deliveryToggle.addEventListener('click', (e) => {
            e.stopPropagation(); const visible = panels.delivery.style.display === 'block'; hideAll(); if (!visible) { this.renderDeliveryPanel(); positionPanel(panels.delivery, deliveryToggle); }
        });
        ...
        const updateCounts = () => {
            ...
            if (cartCount) cartCount.textContent = this.cart.length || 0;
            if (wishlistCount) wishlistCount.textContent = this.wishlist.length || 0;
            if (compareCount) compareCount.textContent = this.compareList.length || 0;
            if (notifCount) notifCount.textContent = (this.recentlyViewed||[]).length || 0;
        };
    ...
    renderCartPanel() {
        ...
        if (!container) return;
        if (this.cart.length === 0) container.innerHTML = '<em>No items in cart.</em>';
        else container.innerHTML = this.cart.map(i => `<div class="mini-item"><strong>${i.title||'Book'}</strong><div>Qty: ${i.quantity||1}</div><div>₨${i.price||0}</div></div>`).join('');
    }

    renderWishlistPanel() {
        const container = document.getElementById('wishlistItemsContainer');
        if (!container) return;
        if (this.wishlist.length === 0) container.innerHTML = '<em>No items in wishlist.</em>';
        else container.innerHTML = this.wishlist.map(i=>`<div class="mini-item">${i.title||'Book'}</div>`).join('');
    }

    renderComparePanel() {
        const container = document.getElementById('compareItemsContainer');
        if (!container) return;
        if (this.compareList.length === 0) container.innerHTML = '<em>No items to compare.</em>';
        else container.innerHTML = this.compareList.map(i=>`<div class="mini-item">${i.title||'Book'}</div>`).join('');
    }

    renderDeliveryPanel() {
        const container = document.getElementById('deliveryStatusContainer');
        if (!container) return;
        if (!this.orderHistory || this.orderHistory.length === 0) {
            container.innerHTML = 'No recent orders.';
            return;
        }

        const statuses = ['Pending','Dispatched','Out for Delivery','Delivered'];

        container.innerHTML = this.orderHistory.slice(0,5).map(o => {
            const idx = statuses.indexOf(o.status) >= 0 ? statuses.indexOf(o.status) : 0;
            const stepsHtml = statuses.map((s,i)=>`<div class="delivery-step ${i<=idx? 'active':''}" title="${s}"></div>`).join('');
            return `
                <div class="order-item" data-order-id="${o.id}">
                    <strong>Order #${o.id}</strong>
                    <div>Placed: ${o.date || 'N/A'}</div>
                    <div class="status-row">Status: <span class="status-badge ${o.status==='Pending'?'status-pending':''} ${o.status==='Dispatched'?'status-dispatched':''} ${o.status==='Out for Delivery'?'status-out':''} ${o.status==='Delivered'?'status-delivered':''}">${o.status}</span></div>
                    <div class="delivery-steps">${stepsHtml}</div>
                    <div class="shipping-info">
                        <label>Shipping Method:
                            <select class="shipping-method">
                                <option${o.shippingMethod==='Standard'? ' selected':''}>Standard</option>
                                <option${o.shippingMethod==='Express'? ' selected':''}>Express</option>
                                <option${o.shippingMethod==='Next-Day'? ' selected':''}>Next-Day</option>
                            </select>
                        </label>
                        <label>Courier:
                            <input type="text" class="shipping-courier" value="${o.courier || ''}" />
                        </label>
                        <label>Tracking #:
                            <input type="text" class="shipping-tracking" value="${o.trackingNumber || ''}" />
                        </label>
                        <label>ETA:
                            <input type="date" class="shipping-eta" value="${o.eta || ''}" />
                        </label>
                    </div>
                    <div class="delivery-controls">
                        <button class="btn-next" data-order="${o.id}">Advance</button>
                        <button class="btn-reset" data-order="${o.id}">Reset</button>
                        <button class="btn-save" data-order="${o.id}">Save</button>
                    </div>
                </div>
            `;
        }).join('');
        ...
        // wire buttons
        container.querySelectorAll('.btn-next').forEach(b=>{
        ...
        container.querySelectorAll('.btn-reset').forEach(b=>{
            b.addEventListener('click', (e)=>{
                const id = e.currentTarget.getAttribute('data-order');
                this.setOrderStatus(id,'Pending');
                this.renderDeliveryPanel();
            });
        });
        ...
        container.querySelectorAll('.btn-save').forEach(b=>{
            b.addEventListener('click', (e)=>{
                const id = e.currentTarget.getAttribute('data-order');
                const orderEl = container.querySelector(`.order-item[data-order-id="${id}"]`);
                if (!orderEl) return;
                const details = {
                    shippingMethod: orderEl.querySelector('.shipping-method')?.value,
                    courier: orderEl.querySelector('.shipping-courier')?.value,
                    trackingNumber: orderEl.querySelector('.shipping-tracking')?.value,
                    eta: orderEl.querySelector('.shipping-eta')?.value
                };
                this.setShippingDetails(id, details);
                this.showNotification(`Shipping details saved for ${id}`, 'success');
                this.renderDeliveryPanel();
            });
        });
    }
    ...
    advanceOrderStatus(orderId) {
        const statuses = ['Pending','Dispatched','Out for Delivery','Delivered'];
        const order = this.orderHistory.find(o=>String(o.id) === String(orderId));
        if (!order) return;
        const cur = statuses.indexOf(order.status) >=0 ? statuses.indexOf(order.status) : 0;
        const next = Math.min(statuses.length-1, cur+1);
        order.status = statuses[next];
    ...
    setShippingDetails(orderId, details = {}) {
        const order = this.orderHistory.find(o => String(o.id) === String(orderId));
        if (!order) return;
        // sanitize and apply details
        if (details.shippingMethod) order.shippingMethod = details.shippingMethod;
        if (details.courier !== undefined) order.courier = details.courier;
        if (details.trackingNumber !== undefined) order.trackingNumber = details.trackingNumber;
        if (details.eta !== undefined) order.eta = details.eta;
        this.saveOrderHistory();
    }
    ...
    generateSampleOrders() {
        // Use consistent human-readable statuses for the delivery UI
        const statuses = ['Pending', 'Dispatched', 'Out for Delivery', 'Delivered'];
        const shippingMethods = ['Standard', 'Express', 'Next-Day'];
        const couriers = ['Postal Lanka', 'ExpressShip', 'QuickCourier', 'IslandPost'];
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            // Randomly choose a status and shipping details
            const status = statuses[Math.floor(Math.random() * statuses.length)];
            const shippingMethod = shippingMethods[Math.floor(Math.random() * shippingMethods.length)];
            const courier = couriers[Math.floor(Math.random() * couriers.length)];
            const trackingNumber = `TRK${Math.floor(100000 + Math.random() * 900000)}`;
            const eta = new Date(Date.now() + (2 + Math.floor(Math.random() * 7)) * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
            ...
            orders.push({
                ...
                items: orderItems,
                total: 25,
                shipping: (shippingMethod === 'Next-Day') ? 9.99 : (shippingMethod === 'Express' ? 7.99 : 4.99),
                tax: 2.00,
    ...
    setupEventListeners() {
        ...
        // Sidebar toggles removed; keep add-to-cart behavior
    ...
    // Notifications removed: UI simplified, notifications handled via modals or server
    // (placeholder kept intentionally empty)
    setupNotifications() {
        // intentionally left empty after UI simplification
    
        // mark-all-read removed along with notifications UI

    // Notification UI removed; notification actions handled elsewhere if needed
    }

    // setupNotificationActions removed; notifications UI simplified

    // markAllNotificationsRead removed; notifications are no longer displayed in header
    ...
    // Set end date for Christmas sale (December 31, 2025)
    ...
    setupLoyaltyProgram() {
        ...
        if (joinBtn) {
            joinBtn.addEventListener('click', () => {
                this.showNotification('Welcome to NETH Bookhive Rewards! You are now earning points on every purchase.', 'success');
    ...
    handleRegister(form) {
        ...
        setTimeout(() => {
            ...
            if (name && email && password && confirmPassword) {
                ...
                this.showNotification('Account created successfully! Welcome to NETH Bookhive!', 'success');
    ...
    // Countdown for holiday sale (updates #days, #hours, #minutes, #seconds)
    startHolidayCountdown() {
        ...
        function update() {
            if (!daysEl) return; // countdown not present
            const now = new Date();
            let year = now.getFullYear();
            let target = new Date(year, 11, 31, 23, 59, 59);
            if (now > target) {
                target = new Date(year + 1, 11, 31, 23, 59, 59);
            }
            ...
            animateIfChanged(daysEl, days, 'days');
            animateIfChanged(hoursEl, hours, 'hours');
            animateIfChanged(minutesEl, minutes, 'minutes');
            animateIfChanged(secondsEl, seconds, 'seconds');
        }

        update();
        this._holidayCountdownInterval = setInterval(update, 1000);
    }
    ...
    // Sidebar functionality removed (UI simplified)
    ...
    // setupSidebars removed after sidebar HTML was removed
    ...
    setupPriceRange() {
        ...
        if (priceRangeMin && priceRangeMax && priceMinSpan && priceMaxSpan) {
            // mark the top slider to accept pointer events
            priceRangeMax.classList.add('top');

            const setRangeFill = () => {
                const min = parseInt(priceRangeMin.min || 0, 10);
                const max = parseInt(priceRangeMax.max || 3000, 10);
                const val1 = parseInt(priceRangeMin.value, 10);
                const val2 = parseInt(priceRangeMax.value, 10);
                const percent1 = ((val1 - min) / (max - min)) * 100;
                const percent2 = ((val2 - min) / (max - min)) * 100;
                const container = priceRangeMin.parentElement;
                if (container) {
                    // paint a background gradient showing unselected / selected / unselected
                    container.style.background = `linear-gradient(90deg, #ddd ${percent1}%, #3498db ${percent1}%, #3498db ${percent2}%, #ddd ${percent2}%)`;
                }
            };
            ...
            priceRangeMin.addEventListener('input', (e) => { updatePriceDisplay(); setRangeFill(); });
            priceRangeMax.addEventListener('input', (e) => { updatePriceDisplay(); setRangeFill(); });
            
            // Apply filters when price changes
            priceRangeMin.addEventListener('change', () => { setRangeFill(); this.applyFilters(); });
            priceRangeMax.addEventListener('change', () => { setRangeFill(); this.applyFilters(); });
...
function filterByCategory(category) {
    ...
    // Notifications dropdown removed from header
    }
}
...
// Notifications removed from header; helper functions omitted to avoid errors
